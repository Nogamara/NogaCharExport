...
if not NogaCharLib then
	...
	function NogaCharLib:GetBaseStats(unit)
		...
		for func, type in pairs(NogaCharLib.fooDefs.baseCallables) do
			if func then
				base[func:gsub("^Get", "")] = mt[func](pUnit)
			end
		end
		
		base.ClassString	= base.ClassId and NogaCharLib.gameDefs.codeEnumClass[base.ClassId] or ""
		base.RaceString		= base.RaceId and NogaCharLib.gameDefs.codeEnumRace[base.RaceId] or ""
		base.FactionString	= base.Faction and NogaCharLib.gameDefs.mappingFactions[base.Faction] or ""
		base.GenderString	= base.Gender and NogaCharLib.gameDefs.mappingGenders[base.Gender] or ""
		base.Realm			= GameLib.GetRealmName()
		base.GearScore		= GameLib.GetGearScore()
	...
	function NogaCharLib:GetMiscStats(unit)
		...
		for func, type in pairs(NogaCharLib.fooDefs.miscCallables) do
			if func then
				misc[func:gsub("^Get", "")] = mt[func](pUnit)
			end
		end
	...
	function NogaCharLib:GetAll(unit)
		local foo = {}
		foo.Base = NogaCharLib:GetBaseStats(unit)
		foo.Misc = NogaCharLib:GetMiscStats(unit)
		foo.Currency = NogaCharLib.GetCurrency()
		
		return foo
	end
	
	_G["NogaCharLib"] = NogaCharLib
